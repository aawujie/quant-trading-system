...
export default function EquityCurve({ backtestResult }) {
  ...
  useEffect(() => {
    ...
    console.log('📈 EquityCurve: containerRef.current =', containerRef.current);
    
    if (!backtestResult || !containerRef.current) {
      console.log('⚠️ EquityCurve: Skipping (backtestResult or container is null)');
      return;
    }
    
    // 防止重复初始化
    if (chartRef.current) {
      console.log('⚠️ EquityCurve: Chart already initialized, skipping');
      return;
    }
    ...
    // 清理
    return () => {
      console.log('🗑️ EquityCurve: Cleaning up chart');
